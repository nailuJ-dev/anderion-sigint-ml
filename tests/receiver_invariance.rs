use anderion_sigint_ml::{cross_receiver_consistency, normalize_receiver_capture, IqCapture, IqSample, ReceiverProfile};

fn tone(cfo: f64) -> IqCapture {
    let sr = 256_000.0;
    let samples = (0..256).map(|n| {
...
fn receiver_profile_removes_declared_cfo_bias() {
    ...
    let profile = ReceiverProfile::new("rx-b").with_cfo_bias_hz(2_500.0).unwrap();
