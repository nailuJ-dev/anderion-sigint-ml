...
pub enum EvidenceEdgeKind { Supports, Contradicts, DerivedFrom }
...
impl SessionEvidenceGraph {
    pub fn add_node(&mut self, kind: impl Into<String>, label: impl Into<String>) -> u64 {
        ...
        self.nodes.push(EvidenceNode { id, kind: kind.into(), label: label.into() });
        id
    }

    pub fn add_edge(&mut self, from: u64, to: u64, kind: EvidenceEdgeKind) -> Result<()> {
        if !self.nodes.iter().any(|node| node.id == from) || !self.nodes.iter().any(|node| node.id == to) {
            return Err(SdkError::InvalidArgument("evidence edge references an unknown node".into()));
        }
    ...
    pub fn why(&self, decision_id: u64) -> Result<WhyResult> {
        let decision = self.nodes.iter().find(|node| node.id == decision_id).cloned()
        ...
        for edge in self.edges.iter().filter(|edge| edge.to == decision_id) {
            let Some(node) = self.nodes.iter().find(|node| node.id == edge.from).cloned() else { continue; };
            match edge.kind {
        ...
        Ok(WhyResult { decision, supporting, contradicting })
    }

    pub fn clear(&mut self) { self.nodes.clear(); self.edges.clear(); self.next_id = 0; }
    pub fn nodes(&self) -> &[EvidenceNode] { &self.nodes }
    pub fn edges(&self) -> &[EvidenceEdge] { &self.edges }
}
