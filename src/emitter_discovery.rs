...
pub enum EmitterHypothesisStatus {
    Known,
    ProvisionalUnknown,
    EnrolledLocal,
    Abstain,
...
impl EmitterDiscoverySession {
    ...
    pub fn hypotheses(&self) -> &[EmitterHypothesis] {
        &self.hypotheses
    }
    ...
    pub fn observe_unknown(&mut self, embedding: &Embedding) -> Result<String> {
        ...
        for (index, hypothesis) in self.hypotheses.iter().enumerate() {
            if hypothesis.prototype.dim() != embedding.dim() {
                continue;
            }
    ...
    pub fn enroll_local(
        ...
        if min_support == 0 {
            return Err(SdkError::InvalidArgument(
                "minimum support must be positive".into(),
            ));
        }
        if !operator_confirmed {
            return Err(SdkError::InvalidArgument(
                "local enrollment requires explicit operator confirmation".into(),
            ));
        }
        let hypothesis = self
            .hypotheses
        ...
        if hypothesis.support_count < min_support {
            return Err(SdkError::InvalidArgument(
                "insufficient evidence for local enrollment".into(),
            ));
        }
