...
impl ReceiverProfile {
    ...
    pub fn with_cfo_bias_hz(mut self, value: f64) -> Result<Self> {
        if !value.is_finite() {
            return Err(SdkError::InvalidArgument("CFO bias must be finite".into()));
        }
        self.cfo_bias_hz = Some(value);
        Ok(self)
    }
}

pub fn normalize_receiver_capture(
    capture: &IqCapture,
...
pub fn cross_receiver_consistency(a: &IqCapture, b: &IqCapture) -> Result<f32> {
    ...
    if a.samples().len() != b.samples().len() {
        return Err(SdkError::DimensionMismatch {
            expected: a.samples().len(),
            actual: b.samples().len(),
        });
    }
    let mut dot = 0.0_f64;
    let mut aa = 0.0_f64;
